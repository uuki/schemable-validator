...
1. **呼び出し箇所を一覧化します。**
   `SV::`、`SchemaBuilder`、`Validator::fromJsonSchema`、`schv_register_schema`、`schv_stored_schema`、`->toValidator()`、`->toJsonSchema()` をプロジェクト全体で検索してください。
   見つかった箇所がそれぞれ、置き換えが必要なバリデーション境界です。

2. **各呼び出し箇所を置き換えます。**
   素の PHP バリデーション（セクション 2）、または任意のライブラリ（セクション 3）で置き換えてください。
   フォーム単位で進めます。スキーマ定義、`validate()` 呼び出し、結果の参照を一つずつ移行してください。
...
4. **WordPress プラグインの登録を削除します**（該当する場合）。
   WP Admin でプラグインを無効化し、`packages/wp-schemable-validator/` またはデプロイ先を削除してください。
   `schv_register_schema()` の呼び出し、Schema Editor の保存スキーマ（`schv_schema_` プレフィックスの `wp_options` キー）、有効テーマ内の `schv-schemas/` ディレクトリも削除してください。

5. **CSRF を置き換えます**（該当する場合）。
   `schv_csrf()` を使用している場合は、WordPress nonce（`wp_create_nonce` / `wp_verify_nonce`）またはスタンドアロンの CSRF ライブラリに置き換えてください。

### 1.2 クライアント

PHP パッケージを削除すると、JSON Schema REST エンドポイント（`/wp-json/schv/v1/schema/*`）もなくなります。
このエンドポイントからスキーマを取得して Zod や Valibot に変換しているクライアントコードは動作しなくなります。

schemable-validator を使用している間、クライアント側の検証ルールは PHP スキーマから実行時に自動で導出されていました。
削除後はサーバーとクライアントの間に共有スキーマソースがなくなるため、各検証ルールをクライアントコード側で独立して定義する必要があります。

1. **`@uuki/schemable-validator-client` のインポート**と `schv/v1/schema` の fetch URL を検索します。

2. **同じルールをクライアントコードで定義します。**
   Zod、Valibot、その他のライブラリを使用してください（セクション 5）。
   セクション 5 の型マッピング表が対応関係を示しています。
...
4. **保守ルールを定めます。**
   サーバー側のバリデーションルールを変更するとき（フィールドの追加、制約の変更など）は、クライアントスキーマにも手動で反映する必要があります。
   共有 JSON Schema がなくなるため、この同期は開発者の責任です。

---

## 2. PHP - 素の PHP によるバリデーション（ライブラリ不要）

サードパーティのバリデーションライブラリが不要な場合は、`SV::object([...])->toValidator()->validate()` を PHP 標準関数によるバリデーション関数に置き換えます。
...
各 `$rule` はエラーメッセージの配列を返すクロージャです（成功時は空配列）。
...
条件付き必須は、Respect/Validation への移行パス（セクション 3.4）と同様に後処理ステップで実装します。

### 2.4 ファイルバリデーション

`SV::file()` は MIME タイプとサイズのチェックをラップしています。
`finfo_file()` による MIME 判定と `$_FILES[...]['size']` によるサイズ制限で置き換えてください。
...
> **デフォルトエンジンに関する注意**
> デフォルトのバリデーションエンジンは `NativeAdapter`（外部依存なし）です。
> デフォルト構成を使用している場合、Respect/Validation の依存はないため削除の必要はありません。
> 本セクションは `RespectAdapter` を明示的に使用しているプロジェクトが対象です。

> **バージョン前提**
> PHP 側は Respect/Validation **2.x** を対象としています。
> 将来このプラグインの依存バージョンが 3.x に上がった際は、別途移行ガイドを追加予定です。
...
schemable-validator を使用している間、PHP スキーマが検証ルールの単一ソースです。
`SchemaBuilder::toJsonSchema()` が JSON Schema を生成し、クライアントパッケージ（または Zod / Valibot アダプター）が実行時にそこから検証ルールを導出します。

削除後はこの自動同期がなくなるため、各検証ルールを PHP とクライアントコードの両方で管理する必要があります。

たとえば、バックエンドの開発者がフィールドを追加したり制約を変更した場合（`minLength` を 1 から 3 に引き上げるなど）、クライアントスキーマも別途更新しなければなりません。
共有 JSON Schema がなければ、このドリフトを自動で検出する仕組みはありません。

運用上の対策として、以下を推奨します。

- 制約を変更するときは、サーバーとクライアントの検証定義を同じ PR に含めてください。
- PHP テストからエクスポートした JSON フィクスチャとクライアントスキーマを比較する CI ステップの導入を検討してください。乖離を早期に検出できます。

以下のセクションで、スタンドアロンのライブラリを使ったクライアント側ルールの書き換え方法を説明します。

---

## 5. クライアント - TypeScript/JavaScript を Zod へ移行

Zod へ移行する場合は JSON Schema 経由のアダプター層をなくし、Zod スキーマを直接定義します。
