...
Respect エスケープハッチ（`RespectRules::rule()`、`RespectRules::postalCode()`、`RespectRules::creditCard()`、`RespectRules::iban()`）や生 `v` スキーマは、マッパブルフィールドに設定したアダプタに関わらず、内部的に RespectAdapter を使用します。
...
- `respect/validation`: `RespectAdapter` と Respect エスケープハッチ（`RespectRules::rule()`、`RespectRules::postalCode()`、`RespectRules::creditCard()`、`RespectRules::iban()`）および生 `v` スキーマを有効化します。
